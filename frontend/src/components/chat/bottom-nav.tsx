...
export function BottomNav() {
  ...
  return (
    ...
    >
      <ul className="mx-auto flex max-w-lg items-stretch justify-between px-3 py-1">
        {tabs.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <li key={href} className="flex-1">
              <Link
                href={href}
                className={`flex flex-col items-center gap-1 rounded-xl px-2 py-1.5 text-[10px] font-medium transition-colors cursor-pointer ${
                  active ? "text-zinc-900 font-semibold" : "text-zinc-400 hover:text-zinc-700"
                }`}
              >
                <Icon active={active} />
                <span>{label}</span>
