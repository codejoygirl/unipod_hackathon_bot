...
def build_evidence_context_xml(evidence_chunks: Sequence[EvidenceChunk]) -> str:
    ...
    for chunk in evidence_chunks:
        ...
        if chunk.locator.message_at is not None:
            attrs.append(
                f'message_at="{html.escape(chunk.locator.message_at, quote=True)}"'
            )
        if chunk.locator.message_end is not None:
            attrs.append(
                f'message_end="{html.escape(chunk.locator.message_end, quote=True)}"'
            )
...
- DATES AND TIMES (critical - two different "clocks"):
  (1) CURRENT TIME in the user message is the only source for what the member means by today, now, tonight, tomorrow, this week, in N minutes/hours/days, etc. in their question (any language). Do the arithmetic from that instant and its IANA timezone (including WAT/CAT/UTC labels in evidence or CURRENT TIME). Example pattern: if now is 18:00 WAT and they ask about "in 10 minutes", the answer anchor is 18:10 WAT that same calendar day unless evidence says otherwise.
  (2) Inside evidence, relative words are relative to WHEN THAT CHAT MESSAGE WAS SENT. Use each <evidence> message_at / message_end attribute, or inline [timestamp] lines - never ingest time or published_at as "today". Convert those words to absolute calendar date/time (honour timezone hints in the text).
  Then compare every resolved event to CURRENT TIME: past, happening now, later today, or on a future calendar day. Answer with absolute dates/times in the member's language. Never treat a decorative calendar emoji as the source of truth.
...
Also check writing quality: re-read the answer once as a careful human editor would -
fix typos, grammar, and accidental language mixing; one language only (the member's ask);
phone-friendly spacing; every listed URL kept intact; dates/times consistent with CURRENT
TIME and evidence message_at (including "in X minutes/hours/days" computed, not guessed).
...
def format_reference_clock(
    *,
    ...
    stamp = now.strftime("%A, %Y-%m-%d %H:%M:%S")

    return (
        f"CURRENT TIME (server clock - trusted):\n"
        f"{stamp} | {abbr} ({utc_label}) | IANA {tz_name}\n"
        "This block is \"now\" for the member's question (today/now/this week and "
        "offsets like in 10 minutes or in two days - compute from this instant).\n"
        "Evidence chat lines have their own send-time: use message_at or inline "
        "timestamps, resolve relative words inside each message, then compare to "
        "CURRENT TIME above."
    )
...
DATE RESOLUTION (mandatory): Relative time in member_question (today, tonight, this week,
in a few minutes, in two days, etc. - any language) anchors to CURRENT TIME only. Compute
offsets from that clock and timezone before answering. Chat evidence uses message_at /
message_end or inline timestamps for when each fact was said; never ingest/publish time.
Resolve every scheduled event to an absolute instant, compare to CURRENT TIME, then answer
(past / today / upcoming) using plain absolute dates/times so members are not confused.
